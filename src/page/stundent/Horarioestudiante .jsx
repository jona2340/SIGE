...
import {
    Box, Typography, Card, Chip, Avatar, IconButton, Tooltip,
    Button, Alert, Skeleton, Drawer, Divider, ListItemButton,
...
import StudentNavbar from '../../components/layout/StudentNavbar.jsx'; // Cambié a StudentNavbar según la importación lógica
import { getStoredUser, logoutUser } from '../../service/authService.js';
import { getHorario } from '../../service/horarioService.js';
...
const COLORES = [
    ...
    { bg: '#fff8e1', border: '#f57f17', text: '#e65100', dot: '#f57f17' },
    { bg: '#efebe9', border: '#6d4c41', text: '#3e2723', dot: '#6d4c41' },
];
...
export default function HorarioEstudiante() {
    ...
    const cargarHorario = async (u) => {
        ...
        setError('');
        try {
            const res = await getHorario(u.carrera, u.cuatrimestre, u.grupo);
            if (res.success && res.data) {
                setBloques((res.data.bloques || []).map((b, i) => ({
                    id: b._id,
                    materiaId: String(b.materia?._id || b.materia),
                    nombre: b.materia?.nombre || 'Materia',
                    docente: b.materia?.docente?.nombre || 'Sin docente asignado',
                    dia: b.dia,
    ...
    // ── Función para IMPRIMIR en el navegador (antigua) ──
    const handlePrint = () => {
        ...
        printWindow.document.write(`
            ...
            <html>
            <head>
                <title>Horario - ${user?.nombre || 'Estudiante'}</title>
                <link href="https://fonts.googleapis.com/css2?family=Montserrat:wght@400;500;600;700&display=swap" rel="stylesheet">
                <style>
                    * { margin: 0; padding: 0; box-sizing: border-box; }
                    body { font-family: 'Montserrat', sans-serif; background: white; padding: 24px; color: #1e1e2d; }
                    .header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; border-bottom: 3px solid #00897b; padding-bottom: 16px; }
                    .header-title { font-size: 22px; font-weight: 700; color: #00897b; }
                    .header-sub { font-size: 13px; color: #666; margin-top: 4px; }
                    .header-meta { text-align: right; font-size: 12px; color: #888; }
                    table { width: 100%; border-collapse: collapse; }
                    th { background: #00897b; color: white; padding: 10px 8px; font-size: 12px; font-weight: 600; text-align: center; }
                    td { border: 1px solid #e0e0e0; padding: 4px; vertical-align: top; min-height: 40px; }
                    td.hora { font-size: 11px; color: #888; text-align: center; width: 60px; background: #fafafa; font-weight: 600; padding: 8px 4px; }
                    .bloque { border-radius: 6px; padding: 6px 8px; margin: 2px 0; font-size: 11px; }
                    .bloque-nombre { font-weight: 700; display: block; }
                    .bloque-docente { font-weight: 400; opacity: 0.8; display: block; margin-top: 2px; }
                    .bloque-hora { font-weight: 500; opacity: 0.75; display: block; font-size: 10px; margin-top: 1px; }
                    .bloque-aula { font-weight: 500; opacity: 0.75; display: block; font-size: 10px; }
                    .footer { margin-top: 16px; font-size: 11px; color: #aaa; text-align: center; }
                </style>
            </head>
            <body>
                ${printContent.innerHTML}
                <div class="footer">Generado por SIGE UTSH · ${new Date().toLocaleDateString('es-MX', { year: 'numeric', month: 'long', day: 'numeric' })}</div>
            </body>
            </html>
        `);
    ...
    const descargarPDFReal = () => {
        // Formato horizontal (landscape) para que quepan bien los 5 días
        const doc = new jsPDF('landscape');

        // Encabezado del PDF
        doc.setFontSize(20);
        doc.setTextColor(0, 137, 123);
        doc.text('Mi Horario de Clases - SIGE UTSH', 14, 20);

        doc.setFontSize(11);
        doc.setTextColor(80, 80, 80);
        doc.text(`Alumno: ${user?.nombre || 'N/A'}`, 14, 28);
        doc.text(`Matrícula: ${user?.matricula || 'N/A'}`, 14, 34);
        doc.text(`${user?.carrera || 'N/A'} · ${user?.cuatrimestre}° Cuatrimestre · Grupo ${user?.grupo}`, 14, 40);

        // Preparamos los encabezados de la tabla
        const head = [['HORA', ...DIAS.map(d => d.toUpperCase())]];

        // Obtenemos todas las horas para iterar por filas
        ...
        const body = [];
        const skipCells = {}; // Lógica para saltar celdas cuando una materia dura > 1 hora
        ...
        horasOrdenadas.forEach((minH, rowIndex) => {
            ...
            DIAS.forEach((dia, colIndex) => {
                // Si la celda está ocupada por una materia de la hora anterior, la saltamos
                if (skipCells[`${rowIndex}_${colIndex}`]) return;

                const bloque = bloques.find(b =>
                    b.dia === dia &&
                    toMin(b.horaInicio) <= minH &&
                    toMin(b.horaFin) > minH
                );

                if (bloque && toMin(bloque.horaInicio) === minH) {
                    // Calculamos cuántas horas dura para fusionar celdas (rowSpan)
                    const durMin = toMin(bloque.horaFin) - toMin(bloque.horaInicio);
                    const rowSpan = Math.ceil(durMin / 60);

                    // Marcamos las celdas de abajo para que sean saltadas
                    ...
                    row.push({
                        ...
                        rowSpan: rowSpan,
                        styles: { fillColor: [227, 242, 253], halign: 'center', valign: 'middle', fontSize: 9 } // Color azul muy claro
                    });
                } else {
        ...
        autoTable(doc, {
            ...
            headStyles: { fillColor: [0, 137, 123], halign: 'center' }
        });

        // Descargar
        doc.save(`Horario_${user?.matricula || 'Alumno'}.pdf`);
    };
    ...
    const drawerContent = (
        <Box sx={{ height: '100%', display: 'flex', flexDirection: 'column', bgcolor: '#1e1e2d', color: 'white' }}>
            <Box sx={{ p: 3, textAlign: 'center', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                ...
                <Typography variant="body2" sx={{ color: '#00897b', fontFamily: fontText, mt: 0.5, fontWeight: 600 }}>{user?.nombre || 'Cargando...'}</Typography>
                    ...
                    <Box sx={{ display: 'grid', gridTemplateColumns: '64px repeat(5, 1fr)', position: 'relative', border: '1px solid #e8eaed', borderTop: 'none', borderRadius: '0 0 12px 12px', overflow: 'hidden' }}>
                        ...
                        <Box sx={{ position: 'relative', height: gridHeight, bgcolor: '#fafafa', borderRight: '1px solid #e8eaed' }}>
                            {ejeHoras.map((hora, i) => (
                                <Box key={hora} sx={{
                                    ...
                                    <Typography sx={{ fontFamily: fontText, fontSize: '0.68rem', fontWeight: 600, color: '#90a4ae' }}>
                                        {hora}
                                    </Typography>
                                </Box>
                            ))}
                            {/* Líneas horizontales en el eje */}
                            {ejeHoras.map((hora, i) => (
                                <Box key={`line-${hora}`} sx={{
                                    position: 'absolute',
                                    top: i * HOUR_HEIGHT,
                                    left: 0, right: 0,
                                    height: '1px',
                        ...
                        {DIAS.map((dia, dIdx) => (
                            <Box key={dia} sx={{
                                position: 'relative',
                                height: gridHeight,
                                borderRight: dIdx < 4 ? '1px solid #e8eaed' : 'none',
                                bgcolor: 'white',
                            }}>
                                ...
                                {bloquesPorDia[dia].map((bloque) => {
                                    ...
                                    return (
                                        <Box key={bloque.id} sx={{
                                            ...
                                            '&:hover': {
                                                boxShadow: `0 4px 16px ${color.border}44`,
                                            ...
                                            <Typography sx={{
                                                fontFamily: fontText, fontWeight: 700,
                                                fontSize: isShort ? '0.68rem' : '0.75rem',
                                                color: color.text,
                                                lineHeight: 1.2,
                                                display: '-webkit-box',
    ...
    // ── Tabla para impresión/PDF ────────────────────────────────────────────
    const renderPrintTable = () => {
        // Construir todas las horas presentes ordenadas
        const horasSet = new Set();
        bloques.forEach(b => {
            for (let t = toMin(b.horaInicio); t < toMin(b.horaFin); t += 60) {
                horasSet.add(t);
        ...
        return (
            <Box ref={scheduleRef} sx={{ display: 'none' }}>
                <div className="header">
                    <div>
                        <div className="header-title">Mi Horario — {user?.nombre}</div>
                        <div className="header-sub">{user?.carrera} · {user?.cuatrimestre}° Cuatrimestre · Grupo {user?.grupo}</div>
                    </div>
                ...
                <table>
                    ...
                    <tbody>
                        {horasOrdenadas.map(minH => {
                            const horaLabel = `${String(Math.floor(minH / 60)).padStart(2, '0')}:00`;
                            const horaFin = `${String(Math.floor((minH + 60) / 60)).padStart(2, '0')}:00`;
                            return (
                                <tr key={minH}>
                                    <td className="hora">{horaLabel}–{horaFin}</td>
                                    {DIAS.map(dia => {
                                        const bloque = bloques.find(b =>
                                            b.dia === dia &&
                                            toMin(b.horaInicio) <= minH &&
                                        ...
                                        const color = bloque ? colorMap[bloque.materiaId] : null;
                                        return (
                                            <td key={dia}>
                                                {bloque && toMin(bloque.horaInicio) === minH && (
                                                    <div className="bloque" style={{ backgroundColor: color?.bg, borderLeft: `4px solid ${color?.border}` }}>
                                                        <span className="bloque-nombre" style={{ color: color?.text }}>{bloque.nombre}</span>
                                                        <span className="bloque-docente" style={{ color: color?.text }}>{bloque.docente}</span>
                                                        <span className="bloque-hora" style={{ color: color?.text }}>{bloque.horaInicio}–{bloque.horaFin}</span>
                                                        {bloque.aula && <span className="bloque-aula" style={{ color: color?.text }}>📍 {bloque.aula}</span>}
                                                    </div>
                                                )}
                                            </td>
                                        );
            ...
            <Box component="main" sx={{ flexGrow: 1, p: { xs: 2, sm: 3, md: 4 }, width: { sm: `calc(100% - ${drawerWidth}px)` }, mt: { xs: 7, sm: 8 } }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: 2, mb: 3 }}>
                    <Box>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 0.5 }}>
                            <CalendarMonthIcon sx={{ fontSize: 28, color: '#00897b' }} />
                        ...
                        <Typography sx={{ fontFamily: fontText, fontSize: '0.85rem', color: '#888', ml: 5.5 }}>
                            {user ? `${user.carrera} · ${user.cuatrimestre}° Cuatrimestre · Grupo ${user.grupo}` : 'Cargando...'}
                        </Typography>
                    </Box>

                    {bloques.length > 0 && (
                        <Box sx={{ display: 'flex', gap: 1 }}>
                            <Tooltip title="Imprimir horario">
                            </Tooltip>
                ...
                {!cargando && bloques.length > 0 && (
                    <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr 1fr', sm: 'repeat(3, 1fr)' }, gap: 2, mb: 3 }}>
                        {[
                            { label: 'Materias', value: materiasUnicas, icon: '📚', color: '#00897b' },
                ...
                {!cargando && bloques.length > 0 && (
                    <Card sx={{ borderRadius: 3, boxShadow: '0 2px 8px rgba(0,0,0,0.04)', mt: 3, overflow: 'hidden' }}>
                        <Box sx={{ bgcolor: '#00897b', px: 3, py: 2 }}>
                            ...
                            {Object.entries(
                                ...
                                return (
                                    <Box key={id} sx={{
                                        ...
                                        border: `1.5px solid ${color.border}20`,
                                        bgcolor: color.bg,
                                        ...
                                        <Box>
                                            ...
                                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mt: 0.3 }}>
                                                <PersonIcon sx={{ fontSize: 11, color: color.text, opacity: 0.65 }} />
                ...
                {/* Tabla oculta para impresión web */}
                {renderPrintTable()}
            </Box>
        </Box>
    );
